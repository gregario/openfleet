...
// Speed profiles for different driving patterns (km/h)
...
function initVehicles(count) {
  return Array.from({ length: count }, (_, i) => ({
    ...
    speed: 0,
    tripState: "parked",
    tripTimer: Math.floor(Math.random() * 5) + 1,
  }));
}
...
function updatePosition(v) {
  ...
  if (v.tripTimer <= 0) {
    if (v.tripState === "parked") {
      ...
      v.speed = p.min + Math.random() * (p.max - p.min);
      v.tripTimer = Math.floor(Math.random() * 30) + 10;
    } else if (v.tripState === "driving") {
      v.tripState = "parked";
      v.speed = 0;
      v.tripTimer = Math.floor(Math.random() * 20) + 5;
    }
  }
...
async function resolveVehicleIds(vehicles) {
  try {
...
async function postPositions(vehicles) {
...
// Only run main() if invoked directly, not when imported for testing
